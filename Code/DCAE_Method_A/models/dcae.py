...
class DCAE(CompressionModel):
    ...
    def compress(self, x):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            support_slices = y_hat_slices[:self.max_support_slices]
            if len(support_slices) > 0:
                # 每個 slice (64ch) 個別過 RWKV，再 concat
                ch_ctx_list = []
                for s in support_slices:
                    ch_ctx_list.append(self.channel_context[slice_index](s))
                ch_ctx = torch.cat(ch_ctx_list, dim=1)
            else:
                ch_ctx = None
